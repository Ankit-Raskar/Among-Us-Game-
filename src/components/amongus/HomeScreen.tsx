    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <h1 className="text-5xl font-black tracking-tight">AMONG <span className="text-yellow-400">US</span></h1>
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6 shadow-2xl">
          <div className="flex gap-2 mb-5 bg-black/30 p-1 rounded-lg">
            <button onClick={() => setTab('create')}
              className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${tab === 'create' ? 'bg-yellow-400 text-black' : 'text-white/70 hover:text-white'}`}>CREATE</button>
            <button onClick={() => setTab('join')}
              className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${tab === 'join' ? 'bg-yellow-400 text-black' : 'text-white/70 hover:text-white'}`}>JOIN</button>
          </div>
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter name (2-14 chars)" maxLength={14}
            className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/30 focus:outline-none focus:border-yellow-400 mb-4" />
          ...
          {tab === 'join' && (
            <>
              ...
              <input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase())} placeholder="ABCDE" maxLength={5}
                className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/30 focus:outline-none focus:border-yellow-400 mb-5 font-mono text-2xl tracking-widest text-center" />
            </>
          )}
